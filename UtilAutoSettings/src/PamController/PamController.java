...
public class PamController implements PamControllerInterface, PamSettings {
	...
	private PamController(int runMode) {
		...
		guiFrameManager = PamGUIManager.createGUI(this);
		guiFrameManager.init(); //perform any start up processes for the GUI. 

		if (!guiFrameManager.hasCallBack()) {
	...
	public void setupPamguard() {
		...
		// Worldwind Map removed 2017/05/05 because the worldwind.jar library duplicates the jsonNode class in jackson-all-1.9.9.jar library, but
		// it isn't the same.  While Eclipse can keep it straight and use class info from jackson instead of worldmap, the PamguardCompiler installer
		// seems to want to use the worldmap instead (even though the classes are listed correctly).  This causes problems when Pamguard tries to
		// load a database with click events that have an error message in them.  It simply stops reading the database when it hits an error message,
		// but does not throw any sort of error itself.
		// If we want to reinstate Worldwind Map, this duplication of code needs to be resolved.
...
//		getMainFrame().revalidate(); //handled inside the GUIFrameManager by notify model changed. The controller should have 
	...
	/**
	 * Get an Array list of PamControlledUnits of a particular class. 
	 * @param unitClass
	 * @return
	 */
...
//		exportDecimusXMLSettings(settingsGroup.getSettingsTime());
	...
	public void toolBarStopButton(PamControlledUnit currentControlledUnit) {
		if (getRunMode() == RUN_PAMVIEW) {
			...
			/*pamStop();*/
			//Will stop, not restart
			UtilAutoSetting.ABORT();
			manualStop = true;
