...
public class SimProcess extends DaqSystem implements PamSettings {
	...
	static SimProcess instance;
	public static SimProcess getInstance()
	{
		return instance;
	}
	...
	private SimArray simArray;
	public SimArray getSimArray()
	{
		return simArray;
	}
	...
	public SimProcess(AcquisitionControl daqControl) {
		...
		{
			UtilAutoSetting.Initialize(this,simSignals);
		}
	...
	public void setupObjects() 
	{
		...
		for (int i = 0; i < n; i++) {
			...
			//TODO Change Simobject before sdu!!!
			//if()
			sdu = new SimObjectDataUnit(this, s, 1000);
			if(UtilAutoSetting.isActive)
				{
					if(s.name.contains("Orca"))
					{
						s = UtilAutoSetting.setupOrca(s);
						System.out.println("changed Orca");
						simParameters.setObject(s, i);
					}
					if(s.name.contains("Boat")) //!Alex
					{ //Change Boat Source level
						s = UtilAutoSetting.setupBoat(s);
						simParameters.setObject(s, i);
					}
				}
...
//	}
	...
	public void setArray() {
		/**EASY Change methode. change current array position to my stuff. 
		 * Hardcoded now, I don't need a panel for that. 
		 */
		simArray = new SimArray(ArrayManager.getArrayManager().getCurrentArray());
		PamVector[] direction = 
			{
					new PamVector(1,1,1), //Left They will be normalized later.
					new PamVector(1,1,1) //right
			};
		//simArray.changeArrayPosition(direction);
		
		//currentArray = simArray.getArray();
		//currentArray.masterLocator.setupLocators(simArray.getArray());
	...
	public SimParameters getcurrentsimParameters()
	{
...
//			generateNoiseQuickly(channelData, nse, i);
			...
			/*Simarray Hydrophone*/
			//phone = simArray.getArray().getHydrophoneArray().get(i).hashCode();
		...
		for (int o = 0; o < nSource; o++) {
			...
			if (nNew >0) {
				...
				GpsData arrayLatLong = ArrayManager.getArrayManager().getCurrentArray().getStreamer(0).getHydrophoneOrigin().getLastStreamerData().getGpsData();
...
//		return "Simulated Data Sources";
	...
	private class SimMouseAdapter extends MouseAdapter {
		...
		private void setPosition(long timeInMillis, LatLong latLong,
			...
			String message = "New Position: "+latLong.formatLatitude()+" "+latLong.formatLongitude()+" "+currentHeight;
			//message+= "\n"+""
			CSVLog.log(message);
		}
