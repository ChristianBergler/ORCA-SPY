...
//		Debug.out.printf("Generate sound at %s or %s diff %d?\n", PamCalendar.formatTime(trueMillis), PamCalendar.formatTime(calMillis), trueMillis-calMillis);
		...
		//TODO SimArray instead of Hydrophone Array. 
		
		PamArray array = ArrayManager.getArrayManager().getCurrentArray();
		/*SimArray Hydrophones, no Idea if this works.*/
		//PamArray array = simProcess.getSimArray().getArray();
		...
		for (int i = 0; i < n; i++) {
			//phoneLatLong is not taken from Array... Where the fuck does this come from :(
			//hydrophoneLocator.
			phoneLatLong[i] = hLocator.getPhoneLatLong(timeMillis, i);
			//phoneLatLong[i] = array.masterLocator.getPhoneLatLong(timeMillis, i);
			...
			//delays[i] = simProcess.getSimArray().getDelay(i, currentPosition, simProcess.getSimulatorSpeedOfSound());//propagationModel.getDelays(); //Screw the propogation bs
		...
		//TODO ONLY ONCE
			//TODO get Boat?? wayne
		if(simSound.getSimObjectDataUnit().getSimObject().name.contains("Orca"))
		{
			LatLong boat = new LatLong();
			//get Boat?
			double dir = boat.bearingTo(simSound.getSimObjectDataUnit().getCurrentPosition());
			String message = "Signal: "+simSound.getSimObjectDataUnit().getSimSignal().getName()+" T-:"+snipStartSample+" Dir: "+dir;
			CSVLog.log(message);
			CSVLog.addOrigin(dir);
