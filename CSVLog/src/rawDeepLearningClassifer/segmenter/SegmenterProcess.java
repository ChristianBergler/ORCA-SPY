...
//					System.out.println("There has been a data overflow. " + PamUtils.getSingleChannel(pamRawData.getChannelBitmap())); 
					...
					if (nextRawChunks[i]== null) {
						long timeMillis = pamRawData.getTimeMilliseconds() + (long) (1000*(copyLen-getBackSmapleHop() )/this.getSampleRate()); 
						long startSample = pamRawData.getStartSample() + copyLen - getBackSmapleHop() ; 
...
//					System.out.println("Pointer to copy from: "  + (currentRawChunks[i].rawData[groupChan].length - dlControl.getDLParams().sampleHop )); 
					...
					int overFlow2 = nextRawChunks[i].copyRawData(currentRawChunks[i].rawData[groupChan], currentRawChunks[i].rawData[groupChan].length - getBackSmapleHop()  , 
	...
	public void pamStart() {
		...
		CSVLog.starttime = System.currentTimeMillis();
	}
